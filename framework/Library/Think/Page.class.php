...
class Page{
    ...
    public function show() {
        ...
        unset($param['p']);
        $form_page = '<form action="'.U(ACTION_NAME, $param).'" method="get"><input type="text" name="p" style="width:30px;" /><input type="submit" value="确定" class="btn-teal small-btn" style="margin-left:0"> </form>';

        //替换分页内容
        $page_str = str_replace(
            array('%HEADER%', '%NOW_PAGE%', '%UP_PAGE%', '%DOWN_PAGE%', '%FIRST%', '%LINK_PAGE%', '%END%', '%TOTAL_ROW%', '%TOTAL_PAGE%', '%FORM_PAGE%'),
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages, $form_page),
            $this->config['theme']);
        return "{$page_str}";
    }
}
