...
class IndexController extends CommonController {
    ...
    public function callBackUrl() {
        if (isset($_GET['code']) && trim($_GET['code'])) {
            ...
            if (isset($res['openid']) && $res['openid']) {
                $openid = $res['openid'];
                //  登陆用户的基本信息
                $user = M('store_wxuser')->where(array('openid' => $openid))->find();
                if ($user) {
                    //  代理id
    ...
    public function getUser() {
        $wxuser = M('store_wxuser')->where(array('openid' => session('wx_user_openid')))->field('id,nickname,headimgurl')->find();
    ...
    public function index() {
        ...
        if (!$open_id) {
            ...
            $wx_user_id = intval(M('store_wxuser')->where(array('openid' => $open_id))->getField('id'));
            if ($wx_user_id) {
    ...
    public function daySign() {
        ...
        $count = M('coupon')->where(array('wxuser_id' => $wx_user_id))->count();
        if ($count == 0) {
            $num = rand(10, 50);
        } else if ($count == 1) {
            $num = rand(5, 20);
        ...
        $limit_money     = sprintf("%.1f", ($num / $rate * 10));
