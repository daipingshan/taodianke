...
class WechatApiController extends CommonController {
    ...
    protected function _getEvent($postObj) {
        ...
        //  首次关注提示信息
        $first_msg = C('STORE_WEIXIN_MP.first_subscribe_msg');
        //  重复关注提示信息
        $repeat_msg = C('STORE_WEIXIN_MP.repeat_subscribe_msg');
        ...
        if ($form_Event == "subscribe" || $form_Event == "SCAN") {
            ...
            if (!$info) {
                ...
                $user->add($user_data);
                $short_url = getShortUrl(C('STORE_WEIXIN_BASE.public_number_url') . U('index/index', array('uid' => $uid)));
    ...
    protected function _getText($postObj) {
        ...
        $proxy_pid = M('store_wxuser')->where(array('openid' => $openid))->getField('proxy_pid');
        ...
        if (!empty(mb_strstr($content, "找")) || !empty(mb_strstr($content, "买"))) {
            ...
            //  请输入图文会话标题
            $title  = array('0' => C('STORE_WEIXIN_MP.title'));
            $picUrl = array('0' => C('STORE_WEIXIN_MP.bg_image'));
            $desc   = array('0' => '宅喵生活，您身边的购物神器！');
            $url    = array('0' => C('STORE_WEIXIN_MP.bg_url'));
            ...
            if (!empty($goods)) {
                foreach ($goods as $k => $v) {
                    ...
                    $desc[$k + 1]   = $v['title'];
                    $url[$k + 1]    = C('STORE_WEIXIN_BASE.public_number_url') . U('Item/index', array('id' => $v['id'], 'uid' => $agent['id']));
                }
            } else {
                $obj      = new \Common\Org\TaoBaoApi();
            ...
            $last_url = C('STORE_WEIXIN_BASE.public_number_url') . U('Item/search', array('keyword' => $keyword, 'uid' => $agent['id']));
            ...
            if ($goods_id != '0') {
                ...
                if (!$goods) {
                    ...
                    $short_url = C('STORE_WEIXIN_BASE.public_number_url') . U('Item/index', array('id' => $goods_id, 'uid' => $agent['id'], 'kou_ling' => $pass_data['data']['kou_ling']));

                    $result = '你要找【' . $tmp['title'] . '】吗？点击查看更多相关内容：' . $short_url . ',祝您购物愉快！';
                } else {
                    $short_url = C('STORE_WEIXIN_BASE.public_number_url') . U('Item/index', array('id' => $goods['id'], 'uid' => $agent['id']));
