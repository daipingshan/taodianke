...
class OrderController extends CommonController {
    ...
    public function GetmyOrder1(){
        ...
        //
        $wwhere="UNIX_TIMESTAMP(pay_time)>=".$ntime." and UNIX_TIMESTAMP(pay_time)<=".$etime." and name='".$user['name']."'  AND (order_status='订单完成' or order_status='订单付款' or order_status='结算完成')";
    ...
    public function Fangxin(){
        if(IS_POST){
            ...
            if($begin_time>=$end_time){
                //$this->ajaxReturn(array('code' => -1, 'error' => '结束时间不能大于小于开始时间！'));
                $this->redirect_message(U('Order/Fangxin'), array('error' => '结束时间不能大于小于开始时间！'));
            }else{
                $pchace=S($key);
                if($pchace){
                    $data=$pchace;
                    ...
                    ini_set('default_socket_timeout',150);
                    $client = new \soapclient("http://j.fei-shou.com:801/Fangxin.asmx?WSDL");
                    $params = array(
                        'commzd' => $newid,
                        'stm'   => $end_time,
                        'setm'   => $begin_time
    ...
    public function Renlin(){
        ...
        $data=array_values($data);
        foreach($data as &$vad){
            $vad['author_id']=$this->author[$vad['author_id']];
        }
        $this->assign('data', $data);
        $this->display();
    }
