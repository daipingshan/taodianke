                    ...
                    <div class="panel-heading">
                        <form>
                            <div class="panel-heading">
                                <div class="row">
                                    <div class="col-sm-4">
                                        <input type="number" name="coupon_sn" value="<?php echo I('get.coupon_sn');?>"
                                               class="form-control" placeholder="请输入券号"/>
                                    </div>
                                    <div class="col-sm-4">
                                        <input type="text" name="time" value="<?php echo ($time); ?>" id="field-1"
                                               class="form-control daterange" placeholder="请选择使用时间范围"/>
                                    </div>
                                    <div class="col-sm-4">
                                        <input type="text" name="nickname" value="<?php echo I('get.nickname');?>"
                                               class="form-control" placeholder="请输入用户昵称"/>
                                    </div>
                                    <br>
                                    <br>
                                    <div class="col-sm-2">
                                        <input type="number" step="0.1" name="min" value="<?php echo I('get.min');?>"
                                               class="form-control" placeholder="抵扣金额min"/>
                                    </div>
                                    <div class="col-sm-2">
                                        <input type="number" step="0.1" name="max" value="<?php echo I('get.max');?>"
                                               class="form-control" placeholder="抵扣金额max"/>
                                    </div>
                                    <div class="col-sm-2">
                                        <select name="check_store_uid" id="check_store_uid" class="form-control">
                                            <option value="">选择门店</option>
                                            <?php if(is_array($store_data)): foreach($store_data as $k=>$v): ?><option value="<?php echo ($v["id"]); ?>"
                                                <?php if(I('get.check_store_uid') == $v['id']): ?>selected<?php endif; ?>
                                    ...
                                    <div class="col-sm-2">
                                        <select name="sort" class="form-control">
                                            ...
                                            <option value="money"
                                            <?php if(I('get.sort') == 'money'): ?>selected<?php endif; ?>
                                            >抵扣金额</option>
                                            <option value="used_time"
                                            <?php if(I('get.sort') == 'used_time'): ?>selected<?php endif; ?>
                                            >使用时间</option>
                                            <option value="add_time"
                                            <?php if(I('get.sort') == 'add_time'): ?>selected<?php endif; ?>
                                            >关注时间</option>
                                        </select>
                                    </div>
                        ...
                        <div class="table-responsive" data-pattern="priority-columns" data-focus-btn-icon="fa-asterisk"
                            ...
                            <table cellspacing="0" class="table table-small-font table-bordered table-striped">
                                ...
                                <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                                        <td><img src="<?php echo ($vo["headimgurl"]); ?>" width="80"/></td>
                                        <td><?php echo ($vo["nickname"]); ?></td>
                                        <td><?php echo ($vo["proxy_name"]); ?></td>
                                        <td><?php echo ($vo["money"]); ?></td>
                                        <td><?php echo ($vo["used_time"]); ?></td>
...
<script>
    var store_id = parseInt("<?php echo I('get.check_store_uid',0,'int');?>");
    var cashier_id = parseInt("<?php echo I('get.check_cashier_id',0,'int');?>");
    if (store_id) {
    ...
    function getCash(store_id, cashier_id) {
        ...
        if (store_id > 0) {
            $.get("<?php echo U('getCashier');?>", {store_id: store_id}, function (res) {
